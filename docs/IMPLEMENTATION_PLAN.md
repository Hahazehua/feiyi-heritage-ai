...
- 默认入口改为 `st.chat_message`、`st.chat_input` 驱动的单页面顾问；
- 对话每轮至多一个问题，最多 5 次主动追问并支持“先为我推荐”；
- 客户主流程隐藏技术模式、赛事术语和内部评估字段；
- 集中式软偏好推断及用户覆盖机制；
- 同页推荐、产品选择、最终方案与下载；
- 明确匿名授权、稳定事件 UUID 和幂等写入；
- SQLite 本地 Repository 与 PostgreSQL/Supabase 适配器；
- 数据库失败不影响业务路径，默认不保存完整聊天原文；
- 新增 `UX_SPEC.md`、`INFERENCE_POLICY.md` 和 `ANALYTICS_SCHEMA.md`。

Wave 2 以下内容保留为历史提交与合规记录，不因客户 UI 隐藏相关表达而删除。
