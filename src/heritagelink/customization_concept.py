...
CONCEPT_DISCLAIMER = "该内容为 AI 整理的定制需求概念，不代表现有产品、正式报价、产能或交付承诺。"
...
def build_customization_concept(
    ...
    return {
        ...
        "target_unit_budget_fen": request.unit_budget_max_fen,
        "quantity": request.quantity,
        "theme_direction": details.customization_theme.strip() or "待商家确认",
        "cultural_requirements": sorted(request.meaning_tags),
        "customization_types": sorted(request.required_types),
        ...
        "required_delivery_days": request.available_lead_days,
