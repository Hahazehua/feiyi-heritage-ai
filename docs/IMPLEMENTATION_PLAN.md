...
UI 修改另运行 `python -m streamlit run app.py --server.headless true` 并手工完成一次核心流程。本次仅文档变更，Python 工具链尚不存在，因此不虚报这些检查已执行。
...
下一次编码应只完成阶段 1：项目配置、类型/枚举、五表加载器和失败优先的 schema 测试。不要同时构建完整 Streamlit 页面。这样能先锁定多商家数据契约，再安全地填充飞颐铁画演示数据和推荐规则。

## 7. 已批准的第三阶段增量（2026-07-14）

在既有阶段完成后增加自然语言需求解析：独立 DeepSeek 客户端、严格本地校验、确定性回退、用户确认界面和全 Mock 测试。该阶段不修改推荐引擎的硬过滤、100 分权重或同分排序，不使用 RAG/向量数据库，并确保无 Key 或 API 故障时详细表单继续工作。
