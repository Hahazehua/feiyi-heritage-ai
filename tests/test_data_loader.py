...
def test_loads_expected_demo_dataset() -> None:
    ...
    claimed = bundle.heritage_items[bundle.heritage_items["official_level"] != "unverified"]
    assert all("国家级" in note or "名录" in note for note in claimed["verification_note"])
    assert all(note.strip() for note in claimed["verification_note"])
